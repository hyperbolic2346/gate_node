...
//			alert(navigator.userAgent);
			...
			*/
			...
			var ELEM_TYPE = 0, ELEM_HREF = 1, ELEM_TITLE = 2, ELEM_GROUP = 3, ELEM_WIDTH = 4, ELEM_HEIGHT = 5, ELEM_HREF_WEBM = 6, ELEM_HREF_OGG = 7, ELEM_HREF_IPAD = 8, ELEM_HREF_IPHONE = 9;
			/**
			...
			*/
			...
			{	
				inst.each( function() {
					...
					inst.elemArray.push(new Array(fileType, $this.attr('href'), $this.attr('title'), $this.data('group'), $this.data('width'), $this.data('height'), $this.data('webm'), $this.data('ogg'), $this.data('ipad'), $this.data('iphone')));
				});
			...
			/**
			* Re-build the list of elements
			*/
			inst.reReadData = function()
			{
				inst.elemArray = new Array();
				inst.options.curElem = -1;					
				jQuery(".html5lightbox").each( function() {
					if (this.nodeName.toLowerCase() != 'a')
						return;
					...
					inst.elemArray.push(new Array(fileType, $this.attr('href'), $this.attr('title'), $this.data('group'), $this.data('width'), $this.data('height'), $this.data('webm'), $this.data('ogg'), $this.data('ipad'), $this.data('iphone')));
					$this.unbind('click').click(inst.clickHandler);
				});
			}
			...
			*/
			inst.showVideo = function(elem)
			{
				...
				inst.resizeLightbox(dataW, dataH, true, function(){
					...
					if (isHTML5)
					{
						var videoSrc = elem[ELEM_HREF_WEBM] ? ele[ELEM_HREF_WEBM] : elem[ELEM_HREF];
						if (inst.options.isIPad && elem[ELEM_HREF_IPAD]) {
							videoSrc = elem[ELEM_HREF_IPAD];
						} else if (inst.options.isIPhone && elem[ELEM_HREF_IPHONE]) {
							videoSrc = elem[ELEM_HREF_IPHONE];
						}
			...
			*/
			...
			inst.checkType = function(href)
			{
				...
				if ( href.match(/\.(flv|mp4|avi|m4v|ogv|ogg|webm)(.*)?$/i) ) 
					return 2;
